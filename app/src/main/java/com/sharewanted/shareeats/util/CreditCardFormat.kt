...
class CreditCardFormat: AppCompatEditText {
    var prefix = ""
        private set
    ...
    private fun init(attrs: AttributeSet?) {
        if (attrs != null) {
            val a = context.theme.obtainStyledAttributes(attrs, R.styleable.CreditCardFormat, 0, 0)
            prefix = a.getString(R.styleable.CreditCardFormat_prefix) ?: prefix
            val separatorStr = a.getString(R.styleable.CreditCardFormat_groupSeparator)
            if (!separatorStr.isNullOrEmpty()) {
                groupSeparator = separatorStr[0]
            }
            numberOfGroups = a.getInteger(R.styleable.CreditCardFormat_numberOfGroups, numberOfGroups)
            groupLength = a.getInteger(R.styleable.CreditCardFormat_groupLength, groupLength)
        }
